...
무손실 정리·문서 최적화·균형 압축·최소 용량·극한 압축의 다섯 프리셋을 제공합니다.
문서 분석 결과로 이미지·폰트·첨부파일 비중과 권장 단계를 표시하며, 일반 손실 프리셋은
저색상·흑백·투명 이미지를 포함해 자동 축소·재압축합니다. 극한 압축은 페이지를 120 DPI
이미지로 다시 구성하므로 링크·양식·주석·텍스트 선택 기능이 사라질 수 있습니다.
전자서명 또는 암호화된 PDF는 원본 보호를 위해 변경하지 않습니다.

압축 파일을 열면 모든 파일이 기본 선택됩니다. `Shift+클릭` 범위 선택, `Ctrl+클릭` 개별
선택과 마우스 오른쪽 버튼 드래그 연속 선택을 지원합니다. 앱 확장자 연결은 Crow Pack을
Windows 연결 후보로 등록하고, 기존 기본 앱이 없는 지원 확장자는 자동 연결합니다.

`도움말`에서 한국어/English를 선택하고 Black(기본), Bright Skyblue, White Pink
화면 테마를 바꿀 수 있습니다. 선택은 현재 Windows 사용자에게 저장됩니다. 앱과 파일 연결
아이콘은 대칭형 검은 까마귀와 압축 박스를 결합한 고해상도 디자인을 사용합니다.
...
`release\CrowPack-v1.5.0-Setup-x64.exe` 설치본이 만들어집니다. 설치는 현재 사용자 영역에
