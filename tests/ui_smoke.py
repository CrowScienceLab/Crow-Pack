...
def main() -> int:
    ...
    def on_loaded(ok):
        ...
        window.web_view.page().runJavaScript(
            ...
            "const englishOk = document.querySelector('[data-i18n=\"nav.info\"]').textContent === 'Help';"
            "if (!document.querySelector('#packSettings .settings-panel')) return false;"
            "if (document.querySelector('#navGroupHome #btnToolUpdate')) return false;"
            "document.getElementById('packToolsButton').click();"
            "if (!document.getElementById('packTools').classList.contains('active')) return false;"
            "if (document.querySelectorAll('.pack-tool-tab').length !== 5) return false;"
            "if (document.querySelector('select#packAction') || document.getElementById('packExpected')) return false;"
            "if (document.documentElement.scrollHeight > innerHeight + 1) return false;"
            "document.getElementById('securityMode').value='private';"
            "document.getElementById('securityMode').dispatchEvent(new Event('change'));"
            "if (document.getElementById('selCompressFmt').value !== '7Z') return false;"
            "applyLanguage('ko');"
