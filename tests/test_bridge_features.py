...
class TestBridgeFeatures(unittest.TestCase):
    ...
    def test_update_status_reports_current_version(self):
        ...
        self.assertTrue(result["success"])
        self.assertEqual(result["version"], "1.0d")
    ...
    def test_pdf_button_backend_uses_lossless_optimizer(self):
        ...
        with (
            ...
            result = json.loads(CoreBridge().optimizePdf())
        self.assertTrue(result["success"])
        self.assertEqual(result["output_path"], output_path)
        optimize.assert_called_once_with(input_path, output_path)
